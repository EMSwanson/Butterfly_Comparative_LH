...
data <- data[data$Provenance=="Reared",]                         ###Limit data to just lab reared individuals
...
data<-na.omit(data.frame("ID"=data$ID,"egg_number"=data$egg_number,"Treatment"=data$Treatment,"species"=data$species, "Hindwing.Perimeter"=data$Hindwing.Perimeter))
...
Pp.data <- data[data$species=="Papilio_polyxenes",]

###Running glms to compare control and treatment
summary(glm(egg_number~Treatment+lHindwing.Perimeter,data=Pr.data,family='poisson'))
summary(glm(egg_number~Treatment+lHindwing.Perimeter,data=Dp.data,family='poisson'))
summary(glm(egg_number~Treatment,data=Va.data,family='poisson'))
summary(glm(egg_number~Treatment,data=Pp.data,family='poisson'))


###ggplot plotting approach
pdf(file="Egg number and treatment reared.pdf",useDingbats=F)  ###Opening a pdf file for writing. set usedingbats to false if you are going to edit
dodge <- position_dodge(width=0.75)
g <- ggplot(data, aes(x = species,fill = Treatment, y= egg_number,xlab="Species")) + stat_boxplot(geom='errorbar',coef=0.5)
...
summary(lm(lLeg.Length~Treatment,data=Pp.data))
